...
1. Set up an org (e.g. a Developer Edition) and apply the following customizations:
   1. Everything described in metadata in `test/package/`. Push to org with `sf deploy metadata -d test/package -o [your-test-org-alias]` or legacy `sfdx force:source:deploy -p test/package -u [your-test-org-alias]`.
