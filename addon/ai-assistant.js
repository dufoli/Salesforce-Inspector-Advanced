...
/**
 * AI Assistant module for Salesforce (ChatGPT, Mistral, Claude, AgentForce)
 * Provides AI-powered features like SOQL query generation and Flow analysis
 */
...
export class AIAssistant {
  ...
  async generateSOQL(description, provider, apiKey, context = {}) {
    ...
    const prompt = this.buildSOQLGenPrompt(description, enhancedContext);

    try {
      const soqlGenContext = {
        systemMessage: "You are a Salesforce SOQL expert. You generate only valid SOQL queries, without explanation.",
        maxTokens: 500
      };

      if (provider === "agentforce") {
        return await this.callAgentForceSOQLGenAPI(context.promptTemplateName, description, enhancedContext);
      } else if (provider === "openai" || provider === "mistral") {
        return await this.callOpenAICompatibleAPI(providerConfig, apiKey, prompt, soqlGenContext);
      } else if (provider === "anthropic") {
        return await this.callAnthropicAPI(providerConfig, apiKey, prompt, soqlGenContext);
      }
...
- Return ONLY the object names, nothing else`;
    ...
    try {
      const objectIdentificationContext = {
        systemMessage: "You are a Salesforce metadata expert. You identify relevant Salesforce custom objects based on query descriptions. Return only object names.",
        maxTokens: 200
      };

      if (provider === "openai" || provider === "mistral") {
        response = await this.callOpenAICompatibleAPI(providerConfig, apiKey, prompt, objectIdentificationContext);
      } else if (provider === "anthropic") {
        response = await this.callAnthropicAPI(providerConfig, apiKey, prompt, objectIdentificationContext);
      } else {
        return [];
      }
   ...
   */
  buildSOQLGenPrompt(description, context) {
...
`;
   ...
   * @param {Object} context - Context object with systemMessage and maxTokens
   */
  async callOpenAICompatibleAPI(providerConfig, apiKey, prompt, context = {}) {
    let currentBrowser;
    ...
    const systemMessage = context.systemMessage || "You are a Salesforce AI assistant. Provide accurate and helpful responses.";
    ...
    return new Promise((resolve, reject) => {
      currentBrowser.runtime.sendMessage({
        ...
        if (!result) {
          reject(new Error("No response generated by AI"));
          return;
        }
   ...
   * @param {Object} context - Context object with systemMessage and maxTokens
   */
  async callAnthropicAPI(providerConfig, apiKey, prompt, context = {}) {
    let currentBrowser;
    if (typeof browser === "undefined") {
      currentBrowser = chrome;
    } else {
  ...
  /**
   * Common method to call the AgentForce (Salesforce Einstein) API
   * @param {Object} params - Parameters to send to the prompt template
   * @param {string} promptTemplateName - Name of the prompt template
   * @returns {Promise<Object>} - The raw API response
   */
  async callAgentForce(params, promptTemplateName) {
    const body = {
      isPreview: false,
      inputParams: {
        valueMap: params
    ...
    try {
      ...
      throw new Error(`AgentForce API error: ${error.message || "Failed to call AgentForce API"}`);
    }
  }

  /**
   * Calls the AgentForce (Salesforce Einstein) API for SOQL generation
   * @param {string} promptTemplateName - Name of the prompt template
   * @param {string} description - Natural language description
   * @param {Object} context - Additional context
   * @returns {Promise<string>} - The generated SOQL query
   */
  async callAgentForceSOQLGenAPI(promptTemplateName, description, context) {
    ...
    // Call the common AgentForce API method
  ...
  /**
   * Builds the prompt for flow description analysis
   * @param {Object} flowMetadata - Flow metadata object containing Label, ApiName, Status, and Metadata
   * @returns {string} - The formatted prompt for AI analysis
   */
  buildFlowDescriptionPrompt(flowMetadata) {
    const {Label, ApiName, Status, Metadata} = flowMetadata;

    return `Analyze this Salesforce Flow metadata and provide a clear description of what this flow does. Focus on the business logic, triggers, decisions, and actions performed by the flow.

Flow Name: ${Label || ApiName}
Flow API Name: ${ApiName}
Flow Status: ${Status}
...
${JSON.stringify(Metadata, null, 2)}`;
  ...
  /**
   * Generates a flow description using AI
   * @param {Object} flowMetadata - Flow metadata object containing Label, ApiName, Status, and Metadata
   * @param {string} provider - AI provider: 'openai', 'mistral', 'anthropic', 'agentforce'
   * @param {Object} options - Optional configuration (promptTemplateName for agentforce)
  ...
  async generateFlowDescription(flowMetadata, provider, options = {}) {
    ...
    if (selectedProvider === "agentforce") {
      if (!promptTemplateName || promptTemplateName.trim() === "") {
        throw new Error("Prompt template name not configured for AgentForce. Please configure it in the options.");
      }
    } else if (!apiKey || apiKey.trim() === "") {
      throw new Error(`API key not configured for ${this.providers[selectedProvider]?.name || selectedProvider}. Please configure it in the options.`);
    }

    const providerConfig = this.providers[selectedProvider];

    // Build the prompt
    const prompt = this.buildFlowDescriptionPrompt(flowMetadata);

    try {
      const flowAnalysisContext = {
        systemMessage: "You are a Salesforce Flow expert. Analyze flow metadata and provide clear, detailed descriptions of what flows do, focusing on business logic, triggers, decisions, and actions.",
        maxTokens: 1000
      };

      if (selectedProvider === "agentforce") {
        return await this.callAgentForceFlowAPI(promptTemplateName, flowMetadata);
      } else if (selectedProvider === "openai" || selectedProvider === "mistral") {
        return await this.callOpenAICompatibleAPI(providerConfig, apiKey, prompt, flowAnalysisContext);
      } else if (selectedProvider === "anthropic") {
        return await this.callAnthropicAPI(providerConfig, apiKey, prompt, flowAnalysisContext);
      }
      throw new Error(`Unsupported AI provider: ${selectedProvider}`);
    } catch (error) {
      console.error("Error generating flow description:", error);
      throw new Error(`Generation error: ${error.message}`);
    }
  }

  /**
   * Calls the AgentForce (Salesforce Einstein) API for flow analysis
   * @param {string} promptTemplateName - Name of the prompt template (e.g., "AnalyzeFlow")
   * @param {Object} flowMetadata - Flow metadata object containing Label, ApiName, Status, and Metadata
   * @returns {Promise<string>} - The generated flow description
   */
  async callAgentForceFlowAPI(promptTemplateName, flowMetadata) {

    // Call the common AgentForce API method
    const response = await this.callAgentForce(flowMetadata, promptTemplateName);

    // Extract flow description from response
    let flowDescription = "";
    if (response.outputParams && response.outputParams.valueMap) {
      flowDescription = response.outputParams.valueMap.description || response.outputParams.valueMap.result || "";
    } else if (response.result) {
      flowDescription = response.result;
    } else if (typeof response === "string") {
      flowDescription = response;
    } else {
      flowDescription = response.description || response.flowDescription || JSON.stringify(response);
    }
    ...
    if (!flowDescription) {
      throw new Error("No flow description generated by AgentForce");
    }
